...
export const Chip: FC<ChipProps> = (props) => {
  ...
  const handleMouseDown = (e: React.MouseEvent<Element>): void => {
    e.preventDefault();
  };

  const disabledFocusedClasses = `${disabled ? ' disabled' : ''}${focused ? ' focused' : ''}`;
  const chipClasses = `rts-chip${disabledFocusedClasses}`;
  const chipIconClasses = `rts-chip-clear-icon${disabledFocusedClasses}`;
  ...
  return (
    ...
    >
      ...
      <svg className={chipIconClasses} viewBox="0 0 512 512" onClick={handleClickIcon}>
        <path
