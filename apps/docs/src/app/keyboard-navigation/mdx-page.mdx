import {
    ...
    dropdownVirtualFocusIdsDefinition,
    skipDropdownVirtualFocusExample,
    virtualFocusIdDefinition
...
Navigation between these elements is controlled via arrow keys and custom `onKeyDown` handler.
...
<DemoContainer code={customVirtualFocusInFieldExample} title="Custom virtual focus in Field">
    <CustomVirtualFocusInFieldExample/>
</DemoContainer>
...
<DemoContainer code={skipDropdownVirtualFocusExample} title="Skip Dropdown virtual focus">
    <SkipDropdownVirtualFocusExample/>
</DemoContainer>
...
- Both can be controlled through the same `onKeyDown` callback and actions (e.g. `focusNext`, `focusPrev`, `focusDropdown`, `focusField`).
