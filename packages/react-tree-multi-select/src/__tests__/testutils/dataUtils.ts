...
export interface TestTreeNode extends TreeNode {
  option: TestOption;
}
...
const mapOptionsToTreeNodes = (
  opts: TestOption[], selected: number[] = [], expanded: number[] = [], disabled: number[] = []
): TestTreeNode[] => {
  return opts.map(option => {
    const treeNode: TestTreeNode = {
      option,
      label: option.name,
...
export const getBaseTreeNodeData = (): TestTreeNode[] => {
  return mapOptionsToTreeNodes(options, [6, 7, 3, 18, 23, 28, 29, 41], [1, 2, 11, 12, 40], [2, 7, 18, 34, 35, 40]);
};

export const getTreeNodeData = (selected?: number[], expanded?: number[], disabled?: number[]): TestTreeNode[] => {
